...
5.  **Generar** redes especiales: estrella, anillo, árbol, rejilla (lattice).
:::

> **Pregunta motivadora:** Si representaras tu grupo de amigos como una red, donde cada persona es un punto (nodo) y cada amistad es una línea, ¿cómo se vería? ¿Quién estaría más "conectado"? ¿Habría grupos aislados?

```{mermaid}
%%| label: fig-mapa-cap1
%%| fig-cap: "Mapa conceptual del capítulo"
flowchart TD
    A([🕸️ Fundamentos de igraph]):::central

    subgraph CREAR["🔧 Crear redes"]
        direction LR
        B1[Nodos y<br/>conexiones]:::crear
        B2[Atributos]:::crear
        B3[Visualización]:::crear
    end

    subgraph GRADO["📈 Grado y distribución"]
        direction LR
        C1[Grado de<br/>entrada / salida]:::grado
        C2[Histogramas]:::grado
    end
...
Un **grafo** (o red) se compone de **vértices** (nodos) y **conexiones** (conexiones entre nodos). En `igraph`, podemos comenzar creando un grafo vacío y luego añadiendo elementos.
...
## Grado (*degree*) y distribución de grado (*degree distribution*){# sec-grado (degree)-basico}
...
%%| fig-width: 8
flowchart TD
    subgraph flujo["Flujo de aprendizaje"]
        A["🔧 1. Crear redes
        <i>make_empty_graph, add_vertices, add_edges</i>"]
        B["✏️ 2. Manipular nodos y aristas
        <i>V(g), E(g), atributos, delete, simplify</i>"]
        C["📊 3. Calcular grado
        <i>degree(), in/out, histograma</i>"]
        D["🔢 4. Matriz de adyacencia
        <i>get.adjacency(), heatmap</i>"]
        E["⭐ 5. Redes especiales
        <i>make_star, make_ring, make_tree</i>"]
    end

    A --> B --> C --> D --> E
    E -.->|"Siguiente capítulo"| F

    F["📈 Distribuciones de grado\n<i>Power-law vs Poisson</i>"]
    ...
    style flujo fill:transparent,stroke:#B4B2A9,stroke-dasharray: 5 5
```
...
**Conexión con el siguiente capítulo:** Ahora que sabes crear redes y calcular grados, en el @sec-grado-distribuciones (degree)-distribuciones exploraremos *cómo se distribuyen* esos grados y por qué algunas redes producen "superhubs" mientras que otras son más homogéneas.
...
::: {.callout-caution title="Respuesta 1" collapse="true"}
...
::: {.callout-caution title="Respuesta 2 " collapse="true"}
...
**Pregunta 3: ¿Qué función usarías para eliminar *self-loops* y conexiones duplicadas de una red?**

::: {.callout-caution title="Respuesta 3" collapse="true"}
`simplify()` con los parámetros `remove.multiple = TRUE` y `remove.loops = TRUE`.
:::

**Pregunta 4: Verdadero o Falso:** ¿La función `add_edges(g, c(1,2,3))` agrega tres conexiones?

::: {.callout-caution title="Respuesta 4" collapse="true"}
❌ **Falso.** El vector debe tener un número par de elementos (pares origen-destino). Este código produciría un error.
:::

**Pregunta 5: ¿Cuántas conexiones tiene un `make_ring(10)`?**

::: {.callout-caution title="Respuesta 5" collapse="true"}
